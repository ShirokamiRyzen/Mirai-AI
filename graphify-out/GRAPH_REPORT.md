...
## Corpus Check
- 86 files · ~112,315 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1639 nodes · 2882 edges · 101 communities (66 shown, 35 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 203 edges (avg confidence: 0.67)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c5e40939`
...
### Community 87 - "De"
Cohesion: 0.15
Nodes (8): createSettings(), De, readText(), resolveURL(), unzip(), upload(), x, x()
...
- **Why does `MiraiNavGraph()` connect `MiraiNavGraph` to `ManagementScreen`, `ChatViewModel`, `SettingsViewModel`, `ModelHubViewModel`, `PersonaViewModel`, `CharacterEditViewModel`, `OpenAiRepository`, `SettingsRepository`, `ChatSessionDao`, `AboutScreen.kt`, `SettingsScreen.kt`, `.onCreate`, `ChatScreen`, `BackupRepository`?**
  _High betweenness centrality (0.071) - this node is a cross-community bridge._
- **Why does `Ue` connect `r` to `index.min.js`, `pixi.min.js`, `ns`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `CharacterEntity` connect `CharacterEntity` to `ManagementScreen`, `.startGeneration`, `ChatSessionDao`, `.buildOpenAiMessages`, `ChatMessageEntity`, `TtsManager`, `SherpaOnnxTtsEngine`, `Context`, `MiraiApplication`, `.streamInference`?**
